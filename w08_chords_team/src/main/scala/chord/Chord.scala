...
object model {
  ...
  trait Chord {
    ...
    def isIncludedBy(filter: String): Boolean = {
      ...
      for (i <- 0 until vektFilter.size) {
        if (toString.contains(vektFilter(i))) { boolContains = true }
      }
      boolContains
    }
  }
  ...
  object Chord {
    ...
    def fromString(chordText: String): Option[Chord] = {
      def isGit(s: String) = if(s.split(":")(0) == "git"){true} else {false}
      def isUku(s: String) = if(s.split(":")(0) == "uku"){true} else {false}
      /**
  ...
  case class Guitar(name: String, grip: Grip) extends Chord { //name = namnet på ackordet?
    override val tuning = "E2 A2 D3 G3 B3 E4".split(" ").toVector

    override def toString: String = s"git:$name:${grip.mkString(" ")}"
  }

  case class Ukulele(name: String, grip: Grip) extends Chord { //name = namnet på ackordet?
