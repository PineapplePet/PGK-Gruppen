...
object ChordPlayer {
  ...
  def apply(chord: Chord, time: Int): Unit = {
    ...
    val tuningInt: Vector[Int] = (for (i <- chord.tuning.indices) yield {
      //tuningInt
    ...
    var noteInt: Vector[Int] = Vector()
    for (i <- chord.tuning.indices) {
      if (chord.grip(i) > -1) {
        noteInt = noteInt :+ tuningInt(i) + chord.grip(i)
      }
    }
    ...
    Thread.sleep(time);
    SimpleNotePlayer.stop; // wait time in milliseconds and than stop the sound
  }
}